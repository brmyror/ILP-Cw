...
public class FlightPathAlgorithm {
    ...
    public static List<LngLat> findPath(LngLat start, LngLat goal, List<RestrictedArea> restrictedAreas) {
        ...
        // no path found -> return straight line (best-effort) if goal not inside buffered areas; else empty
        if (!isInsideAnyArea(goalNode, bufferedAreas)) {
    ...
    private static List<Area> buildBufferedAreas(List<RestrictedArea> restrictedAreas) {
        ...
        for (RestrictedArea ra : restrictedAreas) {
            ...
            float strokeWidth = (float) (0.00015 / 2.0);
    ...
    private static class Node {
        ...
        List<Node> neighbors() {
            // Use 16 compass directions (multiples of 22.5 degrees) to obey allowed headings
